...
class BLIPCir(nn.Module):
    def __init__(
        ...
        # New: Add a projection layer for image prompts
        self.prompt_proj = nn.Linear(text_width, vision_width)
    ...
    def encode_prompt(self, prompt):
        ...
        prompt_output = self.text_encoder(
            prompt_tokens.input_ids,
            attention_mask=prompt_tokens.attention_mask,
            return_dict=True,
            mode=None
        )
        prompt_embeds = prompt_output.last_hidden_state[:, 0, :]  # Use [CLS] token
        return self.prompt_proj(prompt_embeds)

    def forward(self, batch, fabric):
        ref_img, img_prompt, tar_feat, caption, _ = batch
        ...
        prompt_embs = self.encode_prompt(img_prompt)
